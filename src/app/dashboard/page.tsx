...
import { useEffect, useState } from "react";
import { LayoutDashboard, BarChart3, CreditCard, BookOpen, LogOut, Menu, X, Users, DollarSign } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import ManageCourses from "./_components/ManageCourses";
import ManageTransactions from "./_components/ManageTransactions";
...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <div className="flex justify-center items-center h-screen font-black text-slate-400 uppercase tracking-widest animate-pulse">
      Checking Access...
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-gray-900 text-white sticky top-0 z-50 shadow-lg">
        <div className="flex items-center gap-2">
          <div className="bg-blue-500 p-1.5 rounded-lg"><LayoutDashboard size={18} /></div>
          <span className="font-black tracking-tight text-lg">AdminPanel</span>
        </div>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 bg-gray-800 rounded-xl text-blue-400">
          {isSidebarOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>
      ...
      <AnimatePresence>
        {(isSidebarOpen || (typeof window !== 'undefined' && window.innerWidth > 768)) && (
          <motion.aside 
            initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }}
            className="fixed md:sticky top-0 left-0 z-50 w-72 md:w-64 bg-gray-900 text-white h-screen flex flex-col p-6 shadow-2xl"
          >
            <div className="flex items-center gap-2 mb-10">
              <div className="bg-blue-600 p-2 rounded-xl"><LayoutDashboard size={20} /></div>
              <span className="text-xl font-black tracking-tighter">AdminPortal</span>
            </div>
            ...
            <button onClick={() => { localStorage.clear(); window.location.href = "/login"; }} className="flex items-center gap-3 text-red-400 font-black p-4 hover:bg-red-500/10 rounded-2xl transition-all mt-auto border border-red-500/10">
              <LogOut size={20} /> Logout
            </button>
          </motion.aside>
        )}
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <AnimatePresence mode="wait">
          ...
          {activeTab === "transactions" && <ManageTransactions />}
